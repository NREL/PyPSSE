...
class SimSettings(BaseModel):
    ...
    def sim_res_smaller_than_sim_time(cls, v, values):
    ...
    def psse_res_smaller_than_sim_time(cls, v, values):
    ...
    def validate_case_study(cls, v, values):
    ...
    def validate_subscription_file(cls, v, values):
        base_project_path = Path(values["project_path"])
        if v:
    ...
    def validate_user_models(cls, v, values):
        base_project_path = Path(values["project_path"])
        if v:
            paths = []
            for file in v:
    ...
    def validate_simulation_mode(cls, v, values):
...
class SimulationSettings(BaseModel):
    ...
    @validator("export")
    def validate_export_paths(cls, v, values):
