...
class Handler:
    ...
    async def post_psse(self, request):
        ...
        try:
            ...
            result = {"Status": "Success", "Message": "Starting PSSE", "UUID": psse_uuid}

            return web.Response(text=json.dumps(result), status=200)

        except Exception as e:
            return web.Response(text=json.dumps({"Status": "Failed", "Message": str(e)}), status=500)
    ...
    async def delete_psse(self, request):
        ...
        try:
            ...
            return web.Response(
                text=json.dumps(
                    {"Status": "Success", "Message": "Attempting to close psse instance", "UUID": psse_uuid}
                ),
                status=200,
            )
        except Exception:
    ...
    async def get_instance_uuids(self):
        ...
        return web.json_response({"Status": "Success", "Instances": uuids})
    ...
    async def get_instance_status(self, request):
        ...
        return web.Response(text=json.dumps({"Status": status, "Message": msg, "UUID": psse_uuid}), status=status_code)
